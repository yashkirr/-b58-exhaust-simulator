...
The original synthetic preview used a generic pulse oscillator. Version 2 replaces it with a physics-informed pulse-train/resonator pipeline:
...
For the most credible result, use **My B58 recording**. The app preserves the real source timbre and applies only the estimated parallel-path transfer. Exact installed sound still requires fabrication or a calibrated 1-D engine gas-dynamics model with measured source impedance and muffler data.
